...
for i in range(number_of_element):
 ...
 for e in events:
	scheduler.add_job(tick, 'date', run_date=datetime(e.year, e.month, e.day, e.hour, e.minute), coalesce=True, args=[])
        ...
        line_bot_api.reply_message(
            ...
            TextSendMessage(
                ...
                quick_reply=QuickReply(
                    items=[
                        QuickReplyButton(
                            action=CameraAction(label="Take Picture")
                        ),
#                         QuickReplyButton(
#                             action=PostbackAction(label="label1", data="data1")
#                         ),
                        QuickReplyButton(
                            action=MessageAction(label="Instruction", text="instruction")
                        ),
                        # QuickReplyButton(
...
#                                                         mode="date")
                        # ),
                        QuickReplyButton(
                            action=MessageAction(label="Schedule", text="schedule")
                        ),
                        QuickReplyButton(
                            action=CameraRollAction(label="Upload Photo")
                        ),
                        QuickReplyButton(
...
def handle_join(event):
    line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(
                text='You can call following small buttons at anytime by simply typing "Button"',
                quick_reply=QuickReply(
                    items=[
                        QuickReplyButton(
                            action=CameraAction(label="Take Picture")
